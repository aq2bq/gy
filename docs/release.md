...
> When the dry-run is repeated with the same version number, the gy-ledger artifacts built in the previous verification (`target/`, `target/package/*-<version>` and `~/.cargo/registry/src/*/gy-ledger-<version>`) are reused, and the verification of `gy` or `gy-serve` can fail against an old API. The failure names the crate being verified, not the stale one: on 2026-09-20 it read `unresolved import gy_ledger::Retraction` and `no field body_marked on type Shown` while both were in the source. Check crates.io first — a version that is genuinely taken cannot be reused — and when it is free, run `cargo clean`, delete those directories, and do the dry-run again (confirmed on 2026-09-15 while preparing 0.5.0, and again on 2026-09-20 for 1.0.0).
