...
`publish` は、指定した時点と範囲の記録と診断を 1 つの Markdown ファイルに出す。開発の成果物としてコミットし、後から判断と経緯を振り返る。使うのはエージェントで、マスターが読む読み物ではない（d-edb0）。

- 見出し: 生成日時、ログの seq、scope と since、書き手（`GY_ACTOR`）、正本の場所。
- 読み方: ノードの種類、要求の 4 状態、12 の関係と向き、閉じ方、ID と別名、履歴の項目。この節だけで以降を読める。
- 記録: 範囲内の全ノード（`--since` なら、その seq より後に変更のあったノード）。scope、種類、created、ID の順に並べる。各ノードは逐語で省略しない。参照はすべて ID（別名）と題名を伴う。
- 履歴: 範囲内の書き込み単位を時系列で。各行にノードの題名を添える。
- 診断: その時点の handover の errors と warnings を件数と一覧で。

「いま判断待ち」「未決の論点」は出さない。どちらも記録から導けるもので、振り返りには要らない。
