...
### needs
- [n-54b9 (N-12) 母数の概念が無いゲートを、数値を作らずに記録できるようにする](needs/n-54b9-母数の概念が無いゲートを-数値を作らずに記録できるようにする.md) — closed
- [n-ce3c (N-11) 設計の時点で名前が確定しない対象を、範囲の検査に載せられるようにする](needs/n-ce3c-設計の時点で名前が確定しない対象を-範囲の検査に載せられるようにする.md) — closed
- [n-f2e8 (N-13) 記録の各項目が何を書く場所かを、機械が読める場所に持たせる](needs/n-f2e8-記録の各項目が何を書く場所かを-機械が読める場所に持たせる.md) — closed
### questions
...
| 71 | 2026-09-15 10:21 | lead | n-54b9 (N-12) 母数の概念が無いゲートを、数値を作らずに記録できるようにする | updated | need close n-54b9 | ゲートは D-68 で第一級概念から外し、gy 0.5 に無い（d-736e） |
| 72 | 2026-09-15 10:21 | lead | n-ce3c (N-11) 設計の時点で名前が確定しない対象を、範囲の検査に載せられるようにする | updated | need close n-ce3c | workflow の記録と範囲の検査は D-60 / N-35 で消した（d-736e） |
| 73 | 2026-09-15 10:21 | lead | n-f2e8 (N-13) 記録の各項目が何を書く場所かを、機械が読める場所に持たせる | updated | need close n-f2e8 | workflow の記録の型は D-60 / N-35 で消した（d-736e） |
| 87 | 2026-09-15 10:22 | lead | ac-1b8a (AC-15) 母数の概念が無いゲートの合格を、数値を作らずに記録でき、母数のあるゲートでは母数の欠落が検出される | updated | edit ac-1b8a | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 94 | 2026-09-15 10:22 | lead | ac-4305 (AC-16) workflow の各記録と各項目に説明を書け、その説明が handover --json で読める | updated | edit ac-4305 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 99 | 2026-09-15 10:22 | lead | ac-554a (AC-14) 設計の時点で名前が確定しないファイルを含む設計と実装の報告が範囲の検査を通り、宣言されていないファイルが混ざった場合は落ちる | updated | edit ac-554a | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
