...
| 85 | 2026-09-15 10:22 | lead | ac-0b0b (AC-31) ツールバーが絞り込み・グラフ操作・リセットの3群に分かれて 1440px で 1 行に収まり、状態の表示（焦点・件数・絞り込み）が画面に 1 か所だけあり、凡例がグラフのノードを覆わない | updated | edit ac-0b0b | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 86 | 2026-09-15 10:22 | lead | ac-1967 (AC-2) 俯瞰から個別描画へ至る経路が、ノードIDを知らない利用者の操作だけで成立する | updated | edit ac-1967 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 88 | 2026-09-15 10:22 | lead | ac-1da3 (AC-6) 分割の前後で、同じ台帳から生成されるHTMLが同一である | updated | edit ac-1da3 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 89 | 2026-09-15 10:22 | lead | ac-1dcb (AC-5) 実台帳314ノードで、生成したHTMLがネットワークへ出ずに file:// で全機能動作する | updated | edit ac-1dcb | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 90 | 2026-09-15 10:22 | lead | ac-1fe0 (AC-7) 個別描画のノードをクリックすると焦点がそのノードへ移り、直前の焦点へ戻せる | updated | edit ac-1fe0 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 93 | 2026-09-15 10:22 | lead | ac-3e7f (AC-25) ノードをクリックしてもグラフの配置と焦点は変わらず詳細だけが開き、焦点移動と絞り込みは押す前に結果が読める明示ボタンで行われる | updated | edit ac-3e7f | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 95 | 2026-09-15 10:22 | lead | ac-4392 (AC-9) 自動化されたE2Eが、クリックの到達と表示対象の変化に伴う再フィットを検出する | updated | edit ac-4392 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 96 | 2026-09-15 10:22 | lead | ac-4847 (AC-34) 個別描画で辺ラベルの矩形がノードのタイトル矩形と交差せず、交差する場合は辺ラベルを出さずに選択した辺だけで表示する | updated | edit ac-4847 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 100 | 2026-09-15 10:22 | lead | ac-577d (AC-10) 詳細を開いたとき、詳細領域の幅がグラフ領域の幅の0.9倍から1.1倍の範囲に入る（広い画面） | updated | edit ac-577d | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 101 | 2026-09-15 10:22 | lead | ac-57a4 (AC-13) 未知属性が既知の宣言と分けた領域に残り、値が省略されない | updated | edit ac-57a4 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 102 | 2026-09-15 10:22 | lead | ac-5b53 (AC-24) gy.html#<ID> を開くとそのノードの詳細が開いており、焦点・選択・絞り込み・検索語の変更が hash に反映され、ブラウザの戻るで直前の表示状態に戻る | updated | edit ac-5b53 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 104 | 2026-09-15 10:22 | lead | ac-670c (AC-3) 表示対象が変わったときに自動でフィットし、その直後のノードラベルの文字高が11ピクセル以上である | updated | edit ac-670c | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 105 | 2026-09-15 10:22 | lead | ac-6943 (AC-8) 現在どこを見ているかが画面に出ており、複数段戻ることができる | updated | edit ac-6943 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 111 | 2026-09-15 10:22 | lead | ac-8544 (AC-29) 一覧と詳細でタイトル全文が読め、個別描画のラベルは折り返しで全文または先頭全角20文字以上が読める（実台帳で測る） | updated | edit ac-8544 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 114 | 2026-09-15 10:22 | lead | ac-8d4d (AC-33) location.spec の履歴復元テストが単独 10 回連続と全件 3 回連続で成功し、goBack 直後の hash と表示状態の一致が待機に依存しない形で検査される | updated | edit ac-8d4d | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 115 | 2026-09-15 10:22 | lead | ac-92e9 (AC-1) 個別描画で、辺の長さの中央値を最近傍距離の中央値で割った値が5未満である | updated | edit ac-92e9 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 117 | 2026-09-15 10:22 | lead | ac-9e06 (AC-32) クリックハンドラを持つ全要素が button/link の役割とポインタカーソルを持ち、Overview の数値カードはそれぞれの一覧へ至るか枠を持たず、ポインタカーソルを持つのに何も起きない要素が 0 件である | updated | edit ac-9e06 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 118 | 2026-09-15 10:22 | lead | ac-a6d0 (AC-28) 詳細の本文で GFM の表・入れ子リスト・コードブロック・リンクが台帳本文どおりに描かれ、埋め込みエスケープの既存テストが通る | updated | edit ac-a6d0 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 122 | 2026-09-15 10:22 | lead | ac-b0f1 (AC-11) 型・スコープ・状態・充足・閉じ方・置き換え済みが、生のキーと値の行ではなくバッジまたはラベルとして描かれる | updated | edit ac-b0f1 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 124 | 2026-09-15 10:22 | lead | ac-b77a (AC-26) 検索欄と種別・スコープ・状態の絞り込みがヘッダー直下に常に見え、左パネルは Overview / Blockers / Progress だけになる | updated | edit ac-b77a | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 125 | 2026-09-15 10:22 | lead | ac-bd5c (AC-4) 描画件数と非表示件数が、俯瞰と個別の両モードでバナーに出る | updated | edit ac-bd5c | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 130 | 2026-09-15 10:22 | lead | ac-d108 (AC-12) 成立範囲と本文が宣言の一覧と別の領域にあり、1行の文字数が日本語は全角30から45、欧文は45から90に収まる | updated | edit ac-d108 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 133 | 2026-09-15 10:22 | lead | ac-d906 (AC-30) 画面のソースが ui/ 配下の関心ごとのモジュールにあり、グローバル変数の共有が無く、bun build の成果物がコミット済みで CI が再現一致を検査し、再構成の前後で e2e の全テストが通る | updated | edit ac-d906 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
| 135 | 2026-09-15 10:22 | lead | ac-e850 (AC-27) 種別・スコープ・状態で絞ると結果が一覧（表）として出て、クラスタや種別のクリックはその一覧へ至り、各行から1クリックで詳細が開く | updated | edit ac-e850 | 本文（測り方）が空の受け入れ条件を埋める（片付け） |
