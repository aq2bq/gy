...
N-17 の全変更系コマンド（CLI と MCP 共通）。changed_attributes は保存前後で値が変わった属性名（作成時は全属性名）、body_changed は本文が変わったかの真偽値、同値の再設定は空配列と false。属性の値と本文は返さない。req compress の evidence なしは参照操作として既存の全文プレビューを維持し、evidence ありは要約を返す。D-34 を具体化する決定であり、判定の根拠は「結果に残るのは操作が新たに生んだ情報」。
