...
function focusedSelection() {
  ...
  const focus = currentFocus();
  if (!focus.id || candidates.length <= MODE_THRESHOLD) return {ids: candidates, omitted: []};
  const key = JSON.stringify([focus.id, focus.radius, candidates]);
  if (key === selectionKey) return selectionCache;
  const distance = new Map([[focus.id, 0]]), queue = [focus.id];
  for (let i = 0; i < queue.length; i++) {
    const id = queue[i], d = distance.get(id);
    if (d >= focus.radius) continue;
    for (const neighbor of Object.keys(adj[id] || {})) {
      if (!distance.has(neighbor)) { distance.set(neighbor, d + 1); queue.push(neighbor); }
    }
  }
  const ranked = [...candidates].sort((a, b) =>
    distance.get(a) - distance.get(b) || degree[b] - degree[a] || (a < b ? -1 : a > b ? 1 : 0));
  selectionKey = key;
  selectionCache = {ids: ranked.slice(0, MODE_THRESHOLD), omitted: ranked.slice(MODE_THRESHOLD)};
  return selectionCache;
}
function visibleNodes() { return focusedSelection().ids; }
