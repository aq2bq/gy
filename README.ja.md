...
ノードのファイルには、別名と ref 付きの ID、タイトル、scope、created、状態、適用範囲、本文、mark 付きの両向きのエッジ（相手の ID・別名・タイトル）、閉じ方と根拠、要求の記録、自由属性を入れます。参照はすべて相手のタイトルを伴うので、各ファイルは単体で分かります。決定のファイルは系譜の関係を先頭に置きます。
