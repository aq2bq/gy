...
gy は、要求が確定するまでの状態を記録します。ある仕事が何に基づいているかを、ニーズ・論点・決定・要求・受け入れ条件のノードと辺のグラフとして持ちます。使い方は一つで、設定するものはほとんどありません。エージェントが作業しながら書き、`publish` が記録をファイルに書き出し、それをコミットして後から読み返します。canonical はリポジトリの外に置きます。gy がネットワークへ出るのは、チームで記録を共有するとき（experimental）だけで、その通信は git を通ります。

## 人間の側で変わること

gy を操作するのは人間ではなく、その人のエージェントです。人間が得るのは、自分の仕事ではなくなる作業です。
...
- どのエージェントも、いつでもリセットできます。リード、要求定義、実装に分担していても、引き継ぎのメモなしでどれでも消せます。状態を持っていたのは、どのエージェントでもないからです。
- 軌道修正は、古い決定の一節だけで済みます。考えを変えたときや、以前の発言と矛盾する依頼をしたとき、新しい決定は、古い決定のどの一節が効力を失うかを引用しなければ書けません。残りは有効のままで、以後のセッションにはその一節が取り消し済みの印付きで見えます。矛盾を gy が見つけるわけではありません。エージェントが古い決定に出会うのは、手に取った作業がその決定と辺で繋がっているからです。
- 人間に残るのは決めることです。目的とニーズ、そしてエージェントが選択肢と推奨を添えて持ってくる問いです。入口のスキル `gy-loop` の 1 行目も "A person cannot escape the critical decisions. gy frees them from everything else." です。

これは作者が毎日使って得た経験で、保証ではありません。gy が検査するのは記録で、記録の外の作業ではありません。

## 試す

手順は 2 つで、どちらもエージェントに話すだけです。

1. エージェントに一度だけ伝えます。

   > `cargo install gy --locked` で gy を入れ、入った crate の `skills/` にあるスキルを、自分がスキルを読む場所（Claude Code なら `~/.claude/skills/`）へコピーして。パスは gy の README の「インストール」と「スキルの置き方」にある。

2. プロジェクトの `CLAUDE.md` か `AGENTS.md` に 1 行足します。

   > このプロジェクトは **gy** で進行状態を管理している。何かを始める・再開する前に `gy-loop` スキルを読み、記録に従う。

あとは、いつもどおりに仕事を頼みます。最初の一度だけ、エージェントが `gy init` を実行し、2 つのことを聞いてきます。要求を自分で読んで承認したいかと、ニーズが不明なときに聞くか進めるかです。リセットの後は記録から再開します。
...
gy はその終盤のためにあります。計画も日程も持ちません。各作業が何に基づいているか、完了と呼ぶ前に何が成り立っていなければならないかを記録し、記録どうしが噛み合わなくなった箇所を報告します。ニーズがすべて閉じたのに満たされていない受け入れ条件や、誰も待っていない論点です。二つの決定が意味のうえで矛盾するかどうかは、gy は判定しません。後から走らせる `lint` はありません。不正な書き込みはその場で拒まれ、注意が要る状態は `handover` が件数で出します。
...
書きは、記録へ自分のトランザクションを足す操作です。それ以外は読みに数え、remote との間で記録を運ぶ 3 つも読みに入ります。
