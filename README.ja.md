...
- 書き手は `user.name / GY_ACTOR` で記録され、`list` と画面にそう出ます。同じエージェント名でも人間が違えば別の書き手です。名前は git がコミットに署名するのと同じもので、git と同じ順（`GIT_AUTHOR_NAME` が先、無ければ `git config user.name`）で決まります。1 回の書きを台帳と git の履歴が別々の人の名前で指すことはありません。
