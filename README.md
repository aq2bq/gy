...
Graph titles wrap below their IDs for up to three lines; only longer titles are shortened. The table and detail heading retain full titles. Ordinary individual views fit their measured labels into the graph; Lineage preserves readable generation layout and pan access to off-screen records. Edge labels in small individual views avoid node labels and are hidden when no nearby placement fits. Read all relationships in node details.

Overview counts link to their matching records or Blockers findings. The next filter follows the opened HTML data after regeneration. Waiting-reference counts remain plain text because they count relationships. Type chips show selection with a check mark; Lineage states on/off. The single neighborhood action lives in graph navigation, whose Node ID input follows record selection.
