...
A guard may set `waived_by` to the name of a project-defined record; a requirement that carries a valid record of that name is not asked for that guard's records or checks. The waiver is kept in the transition's `waived` map, and an absent or schema-invalid record does not waive the guard.
